      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="{{ $entity }}-inline-create-dialog-label">
            {!! $crud->getSubheading() ?? trans('bo::crud.add').' '.$crud->entity_name !!}
          </h5>
        ...
        @if(view()->exists('vendor.bo.crud.fields.relationship.form_content'))
            @include('vendor.bo.crud.fields.relationship.form_content', [ 'fields' => $fields, 'action' => $action])
        @else
        ...
        <button type="button" class="btn btn-secondary" id="cancelButton">{{trans('bo::crud.cancel')}}</button>
          <button type="button" class="btn btn-primary" id="saveButton">{{trans('bo::crud.save')}}</button>
        </div>
      </div>
    </div>
  </div>
