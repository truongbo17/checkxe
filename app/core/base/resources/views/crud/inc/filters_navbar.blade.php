<nav class="navbar navbar-expand-lg navbar-filters mb-0 pb-0 pt-0">
      ...
      <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#bp-filters-navbar" aria-controls="bp-filters-navbar" aria-expanded="false" aria-label="{{ trans('bo::crud.toggle_filters') }}">
        <span class="la la-filter"></span> {{ trans('bo::crud.filters') }}
      </button>
      ...
      <div class="collapse navbar-collapse" id="bp-filters-navbar">
        <ul class="nav navbar-nav">
          <!-- THE ACTUAL FILTERS -->
    			...
    			@endforeach
          <li class="nav-item"><a href="#" id="remove_filters_button" class="nav-link {{ count(Request::input()) != 0 ? '' : 'invisible' }}"><i class="la la-eraser"></i> {{ trans('bo::crud.remove_filters') }}</a></li>
        </ul>
      </div><!-- /.navbar-collapse -->
  </nav>
