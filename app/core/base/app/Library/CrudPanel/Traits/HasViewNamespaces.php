...
trait HasViewNamespaces
{
    ...
    /**
     * Return the array of view namespaces for bo components from the Laravel config files.
     * It uses the default `bo.crud.view_namespaces` key or a custom provided key.
     *
     * @param  string  $domain
    ...
    {
        return config($customConfigKey ?? 'bo.crud.view_namespaces.'.$domain) ?? [];
    }
