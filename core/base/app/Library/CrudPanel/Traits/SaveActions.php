...
namespace Bo\Base\Library\CrudPanel\Traits;
...
trait SaveActions
{
    ...
    {
        return config('bo.crud.operations.'.$this->getCurrentOperation().'.defaultSaveAction', 'save_and_back');
    }
    ...
    public function getFallBackSaveAction()
    {
        ...
        //if there is an higher action and that action is not the bo default higher one `save_and_back` we return it.
    ...
    public function setSaveAction($forceSaveAction = null)
    {
        ...
        $showBubble = $this->getOperationSetting('showSaveActionChange') ?? config('bo.crud.operations.'.$this->getCurrentOperation().'.showSaveActionChange') ?? true;

        if (
            $showBubble &&
            session($this->getCurrentOperation().'.saveAction', 'save_and_back') !== $saveAction
        ) {
            \Alert::info(trans('bo::crud.save_action_changed_notification'))->flash();
        }
    ...
    /**
     * This functions register BoCMS default save actions into CRUD.
     *
     * @return array
     */
    public function setupDefaultSaveActions()
    {
        $defaultSaveActions = [
            [
                ...
                'button_text' => trans('bo::crud.save_action_save_and_back'),
            ],
            [
                'name' => 'save_and_edit',
                'visible' => function ($crud) {
                ...
                'button_text' => trans('bo::crud.save_action_save_and_edit'),
            ],
            [
                'name' => 'save_and_new',
                'visible' => function ($crud) {
                ...
                'button_text' => trans('bo::crud.save_action_save_and_new'),
            ],
        ];
