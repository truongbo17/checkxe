...
<button type="button" class="btn btn-outline-primary btn-sm ml-1 add-repeatable-element-button">
    + {{ $field['new_item_label'] ?? trans('bo::crud.new_item') }}</button>
...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    @push('crud_fields_scripts')
        <script>
            ...
            $('[bp-field-name="route_link_input"]').hide();

            $(document.body).on("change", "select", function (e) {
                $(e.target).parent().parent().find('[data-repeatable-input-name=route_name]').val(e.target.value);
                $(e.target).parent().parent().find('[data-repeatable-input-name=route_function]').val(generateRouteFunctionName(e.target.value));
                const route_link = $(e.target).parent().parent().find('.select2-selection__rendered').text();
                $(e.target).parent().parent().find('[data-repeatable-input-name=route_link_input]').val(route_link);
            });

            function generateRouteFunctionName(routeName) {
                if (typeof routeName === 'string') {
                    const arrayRouteName = routeName.split('.');

                    if (arrayRouteName.length === 2) {
                        switch (arrayRouteName[1]) {
                            case 'index' :
                                return 'List ' + arrayRouteName[0].charAt(0).toUpperCase() + arrayRouteName[0].slice(1);
                            case 'create' :
                                return 'Create ' + arrayRouteName[0].charAt(0).toUpperCase() + arrayRouteName[0].slice(1);
                            case 'edit' :
                                return 'Edit ' + arrayRouteName[0].charAt(0).toUpperCase() + arrayRouteName[0].slice(1);
                            case 'destroy' :
                                return 'Remove ' + arrayRouteName[0].charAt(0).toUpperCase() + arrayRouteName[0].slice(1);
                            case 'showDetailsRow' :
                                return 'Show detail row ' + arrayRouteName[0].charAt(0).toUpperCase() + arrayRouteName[0].slice(1);
                            default :
                                return arrayRouteName[0].charAt(0).toUpperCase() + arrayRouteName[0].slice(1) + ' ' + arrayRouteName[1];
                        }
                    } else if (arrayRouteName.length > 2) {
                        let stringGenerateRouteName = '';
                        arrayRouteName.forEach(function (valueItem, index) {
                            stringGenerateRouteName += ' ' + valueItem.charAt(0).toUpperCase() + valueItem.slice(1);
                        });
                        return stringGenerateRouteName;
                    } else {
                        return arrayRouteName[0].charAt(0).toUpperCase() + arrayRouteName[0].slice(1);
                    }
                } else {
                    return routeName;
                }
