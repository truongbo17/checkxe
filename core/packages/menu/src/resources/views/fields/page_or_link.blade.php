...
<div class="row" data-init-function="bpFieldInitPageOrLinkElement">
    ...
    <input type="hidden" value="{{ $entry->{$field['name']['page_id']} ?? '' }}"
           name="{{ $field['name']['page_id'] }}"/>
    <input type="hidden" value="{{ $entry->{$field['name']['link']} ?? '' }}" name="{{ $field['name']['link'] }}"/>

    <input type="hidden" value="{{ $entry->{$field['name']['router_name']} ?? '' }}"
    ...
    <div class="col-sm-9">
        {{-- page slug input --}}
        <div
            class="page_or_link_value page_link {{ (isset($entry) && $entry->{$field['name']['type']} === 'page_link') || (isset($entry) && !$entry->{$field['name']['type']} && !$field['allows_null']) || (!isset($entry) && !$field['allows_null']) ? '' : 'd-none' }}">
            <select
                class="form-control"
        ...
        {{-- internal link input --}}
        <div
            class="page_or_link_value internal_link {{ isset($entry) && $entry->{$field['name']['type']} === 'internal_link' ? '' : 'd-none' }}">
            <input
                type="text"
                class="form-control"
        ...
        {{-- external link input --}}
        <div
            class="page_or_link_value external_link {{ isset($entry) && $entry->{$field['name']['type']} === 'external_link' ? '' : 'd-none' }}">
            <input
                type="url"
                class="form-control"
        ...
        {{-- router name input --}}
        <div
            class="page_or_link_value router_name {{ isset($entry) && $entry->{$field['name']['type']} === 'router_name' ? '' : 'd-none' }}">
            <select
                class="form-control"
                for="{{ $field['name']['router_name'] }}"
                required
            >
